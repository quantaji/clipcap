...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, dim_self: int, dim_ref: int, num_heads: int, bias: bool = True, dropout: float = 0.0):
        ...
        self.scale = (dim_self // num_heads)**-0.5
    ...
    def forward(self, x: torch.Tensor, y: Optional[torch.Tensor] = None, mask: Optional[torch.Tensor] = None):
...
class TransformerLayer(nn.Module):
    ...
    def forward_with_attention(self, x: torch.Tensor, y: Optional[torch.Tensor] = None, mask: Optional[torch.Tensor] = None):
...
class ClipCaptionModel(nn.Module):
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        if isinstance(self.clip_project, MLP):
            return super().parameters(recurse)
        else:
            return self.clip_project.parameters(recurse=recurse)

    def train(self, mode: bool = True):
        self.training = mode
        for module in self.children():
            module.train(mode)
        if isinstance(self.clip_project, TransformerMapper):
            self.gpt.eval()
        return self
